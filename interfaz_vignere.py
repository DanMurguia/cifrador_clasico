...
from tkinter import *
from affine_vignere import get_text_string, cifrado_vigenere, descifrar_vigenere, save_text

class VentanaVigenere(tk.Toplevel):
    def __init__(self,  parent, vent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.parent = parent
        self.title("Cifrado Vigenère")
        self.geometry("640x480")
        self.configure(bg='NavajoWhite2')
        self.filename = ""
        self.textoc=""
        self.var_texto = ""
        self.var_llave = StringVar()
        ...
        self.bar = tk.Entry(self, width = 40,state='disable')
        ...
        lbla = tk.Label(self, text="Introduzca la Llave:",
                        font=('Arial',12,'bold italic')
                        ,background='NavajoWhite2', foreground = 'grey1')
        lbla.grid(row=2,column=0,pady=15)
        
        self.llave = tk.Entry(self, width = 20,textvariable=self.var_llave,
                             font=('Arial',12))
        self.llave.grid(row=2, column=1)
        
        lblb = tk.Label(self, text="LLave inversa:",font=('Arial',12,'bold italic')
                        ,background='NavajoWhite2', foreground = 'grey1')
        lblb.grid(row=3,column=0)
        
        self.inv = tk.Entry(self, width = 20, state='disable',
                             font=('Arial',12))
        self.inv.grid(row=3, column=1)
        ...
        btn_cifrar.grid(row=4, column=0, pady='20')
        
        btn_descifrar = tk.Button(self, text="Descifrar", command=self.descifrar,
                               bg='saddle brown',width=10,height = 2,
                               foreground='white',font=('Arial',10,'bold italic'))
        btn_descifrar.grid(row=4,column=1)
        ...
        btn_borrar.grid(row=4,column=2)
        
        self.muestra = tk.Text(self, width = 60, height = 8, state='disable')
        self.muestra.grid(row=5,column=0, columnspan=2,pady=15,padx=15)
        ...
        btn_guardar.grid(row=5,column=2)
    ...
    def cifrar(self):
        llave = self.var_llave.get()
        self.textoc= cifrado_vigenere(self.var_texto,llave)
        self.muestra.configure(state="normal")
    ...
    def descifrar(self):
        llave = self.var_llave.get()
        self.textoc, llave_inv = descifrar_vigenere(self.var_texto,llave)
        self.inv.configure(state="normal")
        self.inv.delete(0,'end')
        self.inv.insert(END, llave_inv)
        self.inv.configure(state="disable")
        self.muestra.configure(state="normal")
        self.muestra.insert(END, self.textoc)
        self.muestra.configure(state="disable")
    ...
    def guardar(self):
        fileas = filedialog.asksaveasfilename(initialdir = "C:/",
                                             title="Seleccionar archivo",
                                             defaultextension=".vig",
                                  filetypes=(("Vignere",".vig"),
                                             ("Txt",".txt"),("Affine",".aff")))
